...
class OrderManager:
    ...
    def place_orders(self):
        ...
        for order in self.orders:
            ...
            while tries < 30:
                tries += 1
                try:
                    response = self.session.post(ORDERS_URL, data=json.dumps(order), headers=headers)
                    break
                except Exception:
                    print(f'Error in placing order. Retrying in {sleep ** 2 * 10} seconds', end="\r")
            ...
            if response.ok:
                ...
                print(f'Order for query {order["name"]} has been placed. Waiting until order is finalized.')
    ...
    def _wait_for_final_state(self, order_url):
        ...
        while tries < 30:
            ...
            try:
                ...
                # If the query fails, re-try
                print(f'Error in checking order status. Retrying in {sleep ** 2 * 10} seconds', end="\r")
                time.sleep(sleep ** 2 * 10)
                sleep += 1
                continue
    ...
    def _available_quota(self):
        quotas = self.session.get("https://api.planet.com/auth/v1/experimental/public/my/subscriptions").json()
        try:
            remaining = quotas[0]["quota_sqkm"] - quotas[0]["quota_used"]
        except Exception:
            print('\033[1;33m You can not access your quota, make sure you have an active account. \033[0m')
